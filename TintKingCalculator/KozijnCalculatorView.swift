...
struct KozijnCalculatorView: View {
    ...
    /// Verdeelt de 120 cm rolbreedte over de gebruikte foliebreedtes zo dat
    /// de benodigde rollengte zo klein mogelijk is — dat is het hele punt
    /// van meerdere banen naast elkaar snijden: hoe voller je de breedte
    /// benut, hoe minder ver je hoeft door te rollen. Dit is geen simpele
    /// "voeg steeds een baan toe aan de grootste breedte"-gok (dat loopt
    /// vast zodra een breedte tijdelijk niet verder verbetert terwijl een
    /// paar banen verderop wél een sprong voorwaarts geeft), maar een echte
    /// optimalisatie: van elke breedte wordt eerst uitgerekend wat de
    /// bottleneck (langste baan, via bottleneckLength) is bij 1, 2, 3, ...
    /// banen. Daarna wordt gezocht naar de kleinst haalbare gezamenlijke
    /// rollengte waarbij alle breedtes samen nog binnen de 120 cm passen.
    private var laneAllocation: [(width: Double, lanes: Int, totalCm: Double)] {
        ...
        // Voor elke breedte: de bottleneck-lengte bij 1, 2, 3, ... banen
        // (nooit meer banen dan er losse stroken van die breedte zijn — dat
        // helpt toch niet meer, elke baan zou dan gewoon leger blijven).
        var achievable: [Double: [Double]] = [:]
        for entry in entries {
            let lengths = strips[entry.width] ?? []
            let maxLanes = max(lengths.count, 1)
            achievable[entry.width] = (1...maxLanes).map { bottleneckLength(for: lengths, lanes: $0) }
        }

        func lanesNeeded(for width: Double, target: Double) -> Int {
            guard let values = achievable[width] else { return 1 }
            for (index, value) in values.enumerated() where value <= target {
                return index + 1
            }
            return values.count
        }

        // Kandidaat-rollengtes: alle haalbare bottleneck-waarden over alle
        // breedtes heen, van ruim (weinig banen nodig) naar krap (veel banen
        // nodig). Voor elke kandidaat wordt gekeken hoeveel rolbreedte er
        // dan nodig is; de kleinste kandidaat die nog binnen 120 cm past, is
        // de beste — dat geeft zowel de minimale rollengte als het bijpassende
        // aantal banen per breedte.
        let candidates = Set(achievable.values.flatMap { $0 }).sorted(by: >)

        var bestLanes: [Double: Int] = Dictionary(uniqueKeysWithValues: entries.map { ($0.width, 1) })
        for target in candidates {
            var totalWidth = 0.0
            var lanesForTarget: [Double: Int] = [:]
            for entry in entries {
                let n = lanesNeeded(for: entry.width, target: target)
                lanesForTarget[entry.width] = n
                totalWidth += entry.width * Double(n)
            }
            guard totalWidth <= Self.rollWidthCm else { continue }
            bestLanes = lanesForTarget
        }

        return entries.map { (width: $0.width, lanes: bestLanes[$0.width] ?? 1, totalCm: $0.totalCm) }
    }
